...
()=> var email | global = require('./config/email/<emailName>/email.js') | require('./config/globals/<globalName>/global.js'); // conditional defition value ordered respective to variable name

// Global variables
var emailText = fs.readFileSync("CHANGE_MESSAGE_FILE", "utf8");
var fromAddress = transporter.auth.user; // this comes from configurred account
var replyTo = fromAddress; // default to this

var mailOptions = {
 to: "CHANGE_SEND_TO", // use raw data from option or flag, or from a <emailList.json>
 bcc: "CHANGE_BCC",    // raw data from option or flag, or from a <bccEmailList.json>
 from: fromAddress,    // from config account file
 replyTo: replyTo,     // defaults to same value as from Address, which is from config account file
